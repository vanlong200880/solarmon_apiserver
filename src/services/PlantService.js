						...
						for (var j = 0; j < deviceGroup.length; j++) {
							switch (deviceGroup[j].table_name) {
								...
								case 'model_inverter_ABB_PVS100':
									let objDevice = await db.queryForObject("Plant.getDataDeviceEnergy", {
										id_project: rs.id,
								...
								case 'model_sensor_IMT_SiRS485':
									let objDeviceIrradiance = await db.queryForList("Plant.getDataDeviceIrradiance", {
										id_project: rs.id,
										id_device_group: deviceGroup[j].id,
										table_name: deviceGroup[j].table_name
									});
								...
								case 'model_sensor_IMT_TaRS485':
									let objDeviceAmbientEmperature = await db.queryForList("Plant.getDataDeviceAmbientEmperature", {
										id_project: rs.id,
										id_device_group: deviceGroup[j].id,
										table_name: deviceGroup[j].table_name
									});
								...
								case 'model_emeter_Vinasino_VSE3T5':
									let objConsumption = await db.queryForObject("Plant.getmeterConsumption", {
										id_project: rs.id,
										id_device_group: deviceGroup[j].id,
										table_name: deviceGroup[j].table_name
									});
					...
					switch (param.filterBy) {
						case 'today':
							...
							if (param.data_send_time == 3) {
								...
								var dataEnergyToday1h = await db.queryForList("Plant.getDataEnergyHour", { groupInverter });
						...
						case '3_day':
							...
							if (param.data_send_time == 2) {
								for (let i = 0; i < 3; i++) {
									...
									let dataEnergyArr = await db.queryForList("Plant.getDataEnergyFifteenMinutes", { groupInverter });
							...
							if (param.data_send_time == 3) {
								for (let i = 0; i < 3; i++) {
									...
									var dataEnergyArr = await db.queryForList("Plant.getDataEnergyHour", { groupInverter });
									var dataEnergyToday1h = await db.queryForList("Plant.getDataEnergyHour", { groupInverter });
						...
						case 'last_month':
						case 'this_month':
							dataEnergyToday = await db.queryForList("Plant.getDataEnergyThisMonth", { groupInverter });
							break;

						case '12_month':
							dataEnergyToday = await db.queryForList("Plant.getDataEnergy12Month", { groupInverter });
							break;
						...
						case 'lifetime':
							if (!Libs.isBlank(param.total_year) && param.total_year < 1) {
								...
								dataEnergyToday = await db.queryForList("Plant.getDataEnergyLifetime", { groupInverter });
							}
							break;
